...
public final class ExportToFile {
	...
	public static void exportOnSalesProduct(ExportToPdf __pdf, SaleProductData __data) throws Exception {
		if (__data.getProductData().isOnSale() && !__data.getProductData().getProductSizes().isEmpty()) {
			...
			try {
				...
				ExportToFile.LOGGER.error("URL error: " + __data.getProductData().getPhotoUrl(), _e);
				throw _e;
			}
